**ANNOUNCEMENT**: This project is now entirely superseded by https://github.com/BazzaCuda/MinimalistMediaPlayerX/ which is built on the famous MPV API, libmpv. As such it provides superior control over your audio and video-playback experience (and images!), including proper zoom, pan, rotation, frame forwards and backwards(!), subtitles, brightness, contrast, gamma and saturation, screenshots and quick editing, as well as providing superior support for the world's media formats. 

Unless I get any specific requests for amendments (bug fixes), the current release of this [here] version of Minimalist Media Player (built on Microsoft's much more limited Windows Media Player ActiveX control) is likely to be its last. Goodbye, faithful servant!
